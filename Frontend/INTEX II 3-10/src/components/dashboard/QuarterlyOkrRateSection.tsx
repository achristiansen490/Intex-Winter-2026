...
function StatCard({
  ...
  return (
    ...
    >
      ...
      <div style={{ fontSize: 18, fontWeight: 700, color: c.forest }}>{value}</div>
    </div>
  );
}
      ...
      {latest && latest.denominator > 0 ? (
        <>
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 12, marginBottom: 12 }}>
            <StatCard label="Actual rate" value={ratePct != null ? `${ratePct}%` : '—'} accent={c.sageLight} />
                ...
                <thead>
                  <tr style={{ background: c.sageLight }}>
                    {['Quarter', 'Rate', 'Target', `${unitLabel} (num / den)`].map((h) => (
                      <th key={h} style={{ padding: '8px 12px', textAlign: 'left', color: c.forest, fontWeight: 600, whiteSpace: 'nowrap' }}>
                        {h}
                      </th>
                    ))}
                  </tr>
