...
BEGIN
  IF EXISTS (SELECT 1 FROM sys.identity_columns WHERE object_id = OBJECT_ID(N'dbo.program_areas')) SET IDENTITY_INSERT dbo.program_areas ON;
...
(8, N'MNT', N'Maintenance', N'Safehouse facility maintenance, repairs, and upkeep.', N'donations, partner_assignments', 1);
  IF EXISTS (SELECT 1 FROM sys.identity_columns WHERE object_id = OBJECT_ID(N'dbo.program_areas')) SET IDENTITY_INSERT dbo.program_areas OFF;
END
COMMIT;
